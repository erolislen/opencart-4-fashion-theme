...
class Product extends \Opencart\System\Engine\Controller {
	public function index(): void {
		...
		if ($product_info) {
			...
			//Whatsapp Order
			$data['whatsapp_order'] = $this->config->get('config_whatsapp_order');
			$data['whatsapp_phone'] = $this->config->get('config_whatsapp_phone');
			$data['whatsapp_text'] 	= $this->config->get('config_whatsapp_text');
			///////////////


			$product_special = $this->db->query('SELECT date_start, date_end FROM `' . DB_PREFIX . 'product_special` WHERE `date_start` <= "'.date("Y-m-d").'" AND `date_start` != "0000-00-00" AND `date_end` != "0000-00-00" AND `product_id` = "'.(int)$product_id.'"');


			if(!isset($product_special->row['date_end']) || $product_special->row['date_end'] <= date("Y-m-d")){
				$data['date_end'] = false;
			}else {
				$data['date_end'] = $product_special->row['date_end'];
			}
			...
			if ($product_info['youtube_link']) {
				$data['youtube_link'] = $product_info['youtube_link'];
			} else {
				$data['youtube_link'] = false;
			}
